...
while IFS= read -r line
  do
    ...
    if [[ "$is_modified_file" == ' M' || "$is_modified_file" == 'M' ]]; then
        if [[ "$line" == ' M coincidencetest/version.txt' ]]; then
...
installed=$(pip3 freeze | grep coincidencetest | wc -l)
if [[ "$installed" == "1" ]]; then
    pip3 uninstall -y coincidencetest 1>/dev/null 2>/dev/null
fi
...
version=$(cat coincidencetest/version.txt)
logstyle-printf "$green""Committing this version:$reset$bold_cyan v$version$reset" timed-command
git add coincidencetest/version.txt 1>/dev/null 2>/dev/null && \
    ...
    rm coincidencetest/version.txt && \
    ...
    python3 -m twine upload --repository coincidencetest dist/* && \
